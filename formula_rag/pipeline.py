...
class Engine:
    ...
    def query(self, text, parameters=None, condition=None, target=None):
        from .catalog import load_catalog
        ...
        runtime = {'dense_used': self.retriever.dense, 'llm_used': False, 'model_output_rejected': False,
                   'catalog_hash': self.retriever.hash, 'network': 'local_only'}
        ...
        def calculate(identifier):
            ...
            if identifier == 'doppler_max' and 'two_way' in request['conditions']:
                ...
                result = {**base, 'status': 'not_applicable', 'errors': ['当前存在遮挡/非视距或其他传播机制，不能将自由空间损耗作为实际传播损耗。']}
            elif absent:
                questions.extend(CONDITIONS.get(c, '请明确条件：' + c) for c in absent)
        ...
        return {'status': status, 'request': request, 'interpretation': interpretation,
                ...
                'warnings': warnings, 'runtime': runtime}
