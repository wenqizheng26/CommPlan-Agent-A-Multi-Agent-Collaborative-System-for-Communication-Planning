...
填入完整示例 → 解析 → 核对参数、来源和模型假设 → 勾选确认 → 计算。2 GHz、1 km 的结果为 **98.42 dB**。修改输入后须先保存并重新确认；缺项和冲突会阻止计算。
...
明确区间保留上下界，多个离散候选分别计算，不自动取其中一个；仅频率与距离支持该扩展。“大约”但无界限的输入进入页面顶部的“需求确认与补充”，支持部分回答和多项集中提交。真实海面、多径、散射、双程和链路预算超出首版范围。明确请求自由空间基准可以规划，但不会证明真实环境满足自由空间或远场。
