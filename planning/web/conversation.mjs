...
const modeNames={deterministic:'规则核验与合并',llm_grounded:'本机模型理解 · 原话核验',deterministic_fallback:'模型不可用或建议无效 · 规则合并',manual_edit:'用户直接编辑'};
function value(v){if(Array.isArray(v))return v.length?v.map(value).join(' / '):'未填写';if(v&&typeof v==='object')return `${v.value} ${v.unit}`;return String(v??'未填写');}
export function renderConversation(host,state){
 ...
 for(const turn of c?.turns||[]){
  ...
  card.append(el('summary',`第 ${turn.number} 条 · ${turn.kind==='edit'?'直接编辑':turn.applied?'已合并 / 已处理':pending?'待澄清':'疑问已处理（原话保留）'}`));
