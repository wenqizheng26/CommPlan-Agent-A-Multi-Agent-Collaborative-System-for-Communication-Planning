...
function draw(){
 ...
 $('status').textContent=historical?'历史只读':busy?'正在处理':s?(labels[s.status]||s.status):'等待输入';
 ...
 $('live-status').textContent=running?`正在运行：${nodes[running[0]][0]} · 等待后端返回`:
  latest?.phase==='interrupted'?'上次运行观察中断，请以已保存任务状态为准。':latest?.phase==='rejected'?'上次操作未提交，已保存版本保持有效。':s?(labels[s.status]||s.status):'等待任务开始';
 drawDetails();drawTimeline();
 const questions=[...(s?.report?.questions||[]),reviewQuestion(s)].filter(Boolean);$('supplement-questions').replaceChildren(...questions.map(q=>el('p',q.replaceAll('distance_km','路径距离（km）').replaceAll('frequency_ghz','载波频率（GHz）'),'supplement-question')));
 renderConversation($('conversation-log'),s);syncButtons();
}
...
async function submit(action){
 ...
 activeContext=['create','edit','supplement'].includes(action)?{task_id:c.task_id,revision:c.expected_revision+(action==='create'?0:1),state_version:0,status:'RUNNING',request:c.input||current.request,conversation:current?.conversation,report:null,trace:[]}:null;
 ...
 finally{busy=false;const finalGeneration=++pollGeneration;await loadActivity(c.task_id,finalGeneration);draw();}
}
