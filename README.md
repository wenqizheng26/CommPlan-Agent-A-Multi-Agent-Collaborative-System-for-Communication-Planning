...
- 2±0.1 GHz、1 km 的自由空间损耗约为 **97.98–98.84 dB**；这是输入范围对应的计算范围，不是置信区间。
...
页面中已选择的目标和模型条件会作为独立上下文传给需求模型，来源标注为“用户选择”，不要求在原文中重复书写。模型只补充尚未选择的项目，其原文引用仍须逐字核验；选择与原文冲突时继续要求核对。新调用的逐次诊断包含具体拒绝原因，旧历史不会自动改写。
...
解析后核对参数、来源与自由空间假设，确认当前版本并计算，结果约为 **98.42 dB**。

如果距离缺失，系统等待补参；如果随后把频率改为 3 GHz，旧确认与旧结果失效，重新确认后结果约为 **101.94 dB**。历史版本保留用于查看，不作为当前版本的计算依据。
