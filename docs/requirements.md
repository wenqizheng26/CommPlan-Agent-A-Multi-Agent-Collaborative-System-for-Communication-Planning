...
| 编号 | 要求 | 范围解释 |
| --- | --- | --- |
| R01 | 通信传播与链路预算起步，最终生成并比较候选方案 | 首版比较同一任务下的参数组合；具体设备库后接，实际传播模型逐项扩展 |
| R02 | 必须使用多 Agent 和 LangGraph | 一个 Orchestrator Agent，加需求与知识、计算规划、验证与解释三个专业 Agent |
...
| R14 | 先规划，再按项目逐项完成 | 优先消除科学与输入流程问题，再接多 Agent 与方案比较；本轮只整理工程文件 |
| R15 | 当前不通过控制屏幕模拟真实输入来消耗额度 | 优先检查已有 JSON、规则、接口与固定返回夹具；浏览器仅用于排版核对，真实模型评测另行安排 |
...
用户输入自然语言后，程序记录原文与任务版本，识别当前任务类型。需求与知识 Agent 结合原文和检索依据组织字段；结构化抽取 Worker 提出数量与语义绑定，程序核实后形成可编辑草稿。
...
| 需求与知识 Agent | 目标、语义和缺项，附原文及资料依据；组织抽取与检索，不替用户确认未知业务值 |
| 计算规划 Agent | 已登记方法、公式依赖、输入引用、候选生成规则；不能自由生成正式数值或任意公式 |
...
单一已知配置视为单个候选；仅知识问答不生成候选。同一候选内部按公式依赖排序；不同候选可以独立执行，但不能因“多 Agent”要求强制并发。本期首先完成单链路、同一已确认评估口径内的组合比较。
...
下列是工作分解及完成标准，不是已执行记录。每项形成证据后再进入依赖它的下一项；详细编码计划在该项开工前制定。
...
[已有 P1 实施草案](superpowers/plans/2026-09-14-01-applicability.md)继续作为起点，执行前按本文复核。框架接入不得跳过 P1—P4 的科学和确认边界。
...
当前允许先核对已有 JSON、编写和运行无模型测试及接口夹具；不安排屏幕模拟真实输入或重复真实模型调用。未来真实协作验收必须实际运行本地模型并记录角色调用，夹具通过不能冒称真实多 Agent 已通过。

## 10. 当前状态及尚需落实的选择

2026-09-15 本轮只读检查确认：项目虚拟环境尚未安装 LangGraph；七条公式、现有查询和页面仍属旧流程。科学缺陷清单尚未修复。此次文档同步不构成程序验收，不重跑历史模型、浏览器或 82 项单元测试。
