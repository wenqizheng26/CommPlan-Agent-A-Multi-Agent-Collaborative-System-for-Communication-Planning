...
## Stage 4 candidate revalidation after visual redesign

Historical documents and inactive outputs were moved to the external `_archive/2026-09-23-commplan/` with a manifest; C3 commit `03c985f` leaves the Git worktree clean. On that commit, Python 233 OK (1 symlink privilege skip), Node 27 PASS, and `pip check` PASS. A clean source ZIP was rebuilt with SHA-256 `64CA21C1DAE6FF21B51CF7A05333D878CBC76AEFD7D1F97DC6D89CF3AE66942C`; validator passed extraction, fingerprint and HTTP create/confirm/restart smoke.

The same ZIP was unpacked again to a fresh Windows directory with no `.venv`. This host has no `py`/`python` launcher on PATH, so the run supplied its verified Python 3.12 executable through `COMMPLAN_PYTHON`. `setup_planning.cmd` installed pinned Planning dependencies, `pip check` passed, and the extracted venv passed HTTP create/confirm/restart recovery smoke.

Isolated online and real-offline Qwen examples both completed at 98.42059991327963 dB. The online roles all reported `llm`; the offline interpretation was deterministic and calculation/review reported `deterministic_fallback`. The project-owned model was restarted and `/health` and `/v1/models` passed. In the in-app browser, the online and offline flow highlights matched the published 98.42 dB result; the offline LLM node reported “调用已降级”. Light appearance at 1430×804 CSS viewport had no page scroll or flow-panel scroll, and no console warning/error. System-dark visual inspection, target Chrome seven-flow validation and independent final diff review remain external/pending gates; these checks are not claimed PASS here.
